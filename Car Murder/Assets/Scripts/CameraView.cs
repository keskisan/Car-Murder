...
public class CameraView : UdonSharpBehaviour
{
    ...
    {
        buttonCounter++;
        if (buttonCounter >= buttonNames.Length) buttonCounter = 0;


        if (gameManager.playerManager.IsPlayerValidAndIngame(gameManager.playerManager.players[buttonCounter]))
        {
            playerButtons[buttonCounter].gameObject.SetActive(true);
            buttonNames[buttonCounter].text = Networking.GetOwner(gameManager.playerManager.players[buttonCounter].gameObject).playerId + Networking.GetOwner(gameManager.playerManager.players[buttonCounter].gameObject).displayName;
        }
        else
        {
    ...
    private void UpdateCameraPosition()
    {
        ...
        if ((int)gameManager.Status < (int)GAMESTATUS.MAP1) NoGame(); //not playing

        
        if (!gameManager.playerManager.IsPlayerValidAndIngame(cameraPlayer))
        {
            PickANewPlayer();
            return;
        }
        if (!cameraPlayer.IsPlayerInGame())
        {
            PickANewPlayer();
            return;
        }
    ...
    private Player findNextPlayer(int startposition) //everyone local
    {
        ...
        {
            if (gameManager.playerManager.IsPlayerValidAndIngame(gameManager.playerManager.players[i])) //only show players thats ingame ingame
            {
